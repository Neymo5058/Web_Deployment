...
const roles = [
  { value: 'user', labelKey: 'signup.account_type_user' },
  { value: 'organizer', labelKey: 'signup.account_type_organizer' },
];
...
const handleSubmit = async () => {
  ...
  if (form.password !== form.confirmPassword) {
    fieldErrors.value = {
      confirmPassword: t('signup.error_password_mismatch'),
    };
    return;
  }
    ...
    <div class="auth-card">
      ...
      <form @submit.prevent="handleSubmit" novalidate>
        <div class="form-group" :class="{ 'has-error': fieldErrors.fullname }">
          <label for="fullname">{{ t('signup.full_name_label') }}</label>
          <input
            id="fullname"
            v-model.trim="form.fullname"
            type="text"
            :placeholder="t('signup.full_name_placeholder')"
            :aria-label="t('signup.full_name_label')"
        ...
        <div class="form-group" :class="{ 'has-error': fieldErrors.password }">
          ...
          <input
            ...
            :aria-label="t('signup.password_label')"
        ...
        >
          ...
          <input
            ...
            required
          />
          <p v-if="fieldErrors.confirmPassword" class="error" role="alert">
            {{ fieldErrors.confirmPassword }}
          </p>
        </div>

        <div class="form-group" :class="{ 'has-error': fieldErrors.role }">
          <label for="role">{{ t('signup.account_type_label') }}</label>
          <select
            id="role"
            v-model="form.role"
            :aria-label="t('signup.account_type_label')"
          ...
          <p v-if="fieldErrors.role" class="error" role="alert">
            {{ fieldErrors.role }}
          </p>
        </div>
        ...
        <button type="submit" class="btn" :disabled="isLoading">
          ...
          <span v-else>{{ t('signup.cta_signup') }}</span>
        </button>
      </form>

      <p class="signup-text">
        {{ t('signup.have_account') }}
        <RouterLink to="/signin">{{ t('signup.link_signin') }}</RouterLink>
      </p>
    </div>
  </div>
</template>
...
.auth-card {
  width: min(460px, 100%);
  padding: clamp(2.4rem, 5.5vw, 3rem);
...
.auth-card h2 {
  ...
  font-size: clamp(1.85rem, 1.8vw + 1.35rem, 2.2rem);
  font-weight: 700;
